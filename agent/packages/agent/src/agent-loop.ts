...
async function runLoop(
	...
	const MAX_GIT_HINT_FILES = 12;
	const MAX_FOLLOW_IMPORT_FILES = 16;
	const MAX_SIBLING_HINT_FILES = 5;
	const normalizePath = (path: string): string => path.replace(/^\.\//, "");
	const wasEdited = (path: string): boolean => editedPaths.has(normalizePath(path));
	let aliasRootCache: string | null = null;
	const nudgeLastSentAt = new Map<string, number>();

	const getAliasRoot = (): string => {
		if (aliasRootCache !== null) return aliasRootCache;
		aliasRootCache = "src";
		try {
			for (const cfg of ["tsconfig.json", "jsconfig.json"]) {
				if (existsSync(cfg)) {
					...
					if (pm) {
						aliasRootCache = pm[1].replace(/\/$/, "");
						break;
					}
					const bm = raw.match(/"baseUrl"\s*:\s*"([^"]+)"/);
					if (bm) {
						aliasRootCache = bm[1].replace(/\/$/, "");
						break;
					}
				}
			}
		} catch {}
		return aliasRootCache ?? "src";
	};

	const enqueueNudge = (text: string, dedupeKey: string, cooldownMs = 20_000): void => {
		const now = Date.now();
		const lastSent = nudgeLastSentAt.get(dedupeKey) ?? 0;
		if (now - lastSent < cooldownMs) return;
		nudgeLastSentAt.set(dedupeKey, now);
	...
	const recordSuccessfulFileMutation = async (targetPath: string): Promise<void> => {
		...
		try {
			...
			const targetExt = targetName.includes(".") ? "." + targetName.split(".").pop() : "";
			...
			const related = siblings
				.filter((f: string) => {
					...
					return (
						(targetExt !== "" && fext === targetExt) ||
						name.includes(".test.") ||
						name.includes(".spec.") ||
						name.includes(".freezed.") ||
						(targetExt === "" && codeExts.has(fext))
					);
				})
				.slice(0, MAX_SIBLING_HINT_FILES);
			if (related.length > 0) {
	...
	if (expectedFiles.length === 0) {
		...
		const _git = (args: string[]) => {
			try {
				const r = _gSpawn("git", args, { cwd: _cwd, timeout: 1500, encoding: "utf-8" });
		...
		if (_head && _refs) {
			...
			if (_refSha) {
				...
				if (_rf.length > 0) {
					...
					if (expectedFiles.length === 0 && toMerge.length > MAX_GIT_HINT_FILES) {
						toMerge = toMerge.slice(0, MAX_GIT_HINT_FILES);
					}
					if (toMerge.length > 0) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (message.stopReason === "error") {
				if (upstreamRetries < UPSTREAM_RETRY_LIMIT) {
					...
					await emit({ type: "turn_end", message, toolResults: [] });
					enqueueNudge(
						"Transient upstream failure occurred. Resume by calling a tool directly — avoid prose. Only file diffs count toward your evaluation score.",
			...
			if (totalLlmRequests >= 3 && pendingMessages.length === 0) {
				...
				if (avgPace > 10_000 && elapsed > 20_000 && (Date.now() - lastSlowPaceNudgeAt) > 30_000) {
					...
					enqueueNudge(
						"You are averaging " + Math.round(avgPace / 1000) + "s per response — too slow. Shorten your reasoning and call tools faster. " + (topFile && !hasProducedEdit ? "Call `edit` on `" + topFile + "` NOW." : "Keep editing — every file matters."),
						"slow-pace-warning",
						30_000,
					);
				}
			...
			if (hasProducedEdit && pendingMessages.length === 0) {
				...
				if (elapsed >= 60_000 && editedPaths.size < foundFiles.length && uneditedTop.length > 0 && elapsed < GRACEFUL_EXIT_MS - 30_000) {
					...
					if (ratio < 0.6) {
						...
						enqueueNudge(
							`WARNING: ${Math.round(elapsed / 1000)}s elapsed but you have only edited ${editedPaths.size}/${foundFiles.length} target files. Unedited: ${list}. Each unedited file = missed points. Read and edit them NOW — even partial changes score. Do not spend more time on already-edited files.`,
							"low-coverage-midrun-warning",
							25_000,
						);
					}
				}
			...
			if (!hasMoreToolCalls && emptyTurnRetries < EMPTY_TURN_MAX) {
				...
				if (tokenCapped || idleStopped) {
					...
					enqueueNudge(
						...
						tokenCapped ? "empty-turn-token-capped" : "empty-turn-idle-stopped",
						12_000,
					);
					continue;
				}
			...
			if (hasMoreToolCalls) {
				...
				for (let bi = 0; bi < toolResults.length; bi++) {
					...
					if (tr.toolName === "bash" && !tr.isError) {
						...
						if (output.includes("ConnectionRefusedError") || output.includes("Connection refused") || output.includes("ECONNREFUSED")) {
							enqueueNudge(
								"No services available in this environment. Network installs and requests will fail. Proceed with \`read\`, \`edit\`, and \`write\` only — avoid \`npm install\` unless unavoidable.",
								"network-offline-environment",
								45_000,
							);
							break;
						}
						...
						if (
							...
							enqueueNudge(
								"Package installs are slow and often blocked offline. Prefer \`edit\`/\`write\` using the repo's existing stack; skip new installs unless the task explicitly names a dependency.",
				...
				for (let i = 0; i < toolResults.length; i++) {
					...
					if (tr.toolName === "read" && !tr.isError && tc && tc.type === "toolCall") {
						...
						if (readPath && typeof readPath === "string" && isRealUnixFile(readPath)) {
							...
							if (readContent.length > 0 && readContent.length < 200_000 && foundFiles.length < MAX_FOLLOW_IMPORT_FILES) {
								const dir = readPath.includes("/") ? readPath.substring(0, readPath.lastIndexOf("/")) : ".";
								const importRe = /(?:from\s+['"]|import\s+['"]|require\s*\(\s*['"])([.@][^'"]+)['"]/g;
								let im: RegExpExecArray | null;
								const aliasRoot = getAliasRoot();
								while ((im = importRe.exec(readContent)) !== null) {
									if (foundFiles.length >= MAX_FOLLOW_IMPORT_FILES) break;
									const raw = im[1];
									let norm: string;
									if (raw.startsWith("@/")) {
				...
				if (now - lastRereadNudgeAt >= 5_000 && pendingMessages.length === 0) {
					for (const [rp, cnt] of pathReadCounts) {
						if (cnt >= 3) {
							...
							enqueueNudge(
								`You have read \`${rp}\` ${cnt} times — stop re-reading it. ${others.length > 0 ? `Move to a file you have not edited yet: ${others.slice(0, 5).map((f: string) => `\`${f}\``).join(", ")}.` : "Apply \`edit\` or \`write\` on a different file or stop."}`,
								`reread-limit:${normRp}`,
								20_000,
							);
							break;
				...
				if (!hasProducedEdit && explorationCount >= dynamicExploreCeiling && pendingMessages.length === 0) {
					enqueueNudge(
						`Context gathered (${explorationCount} reads/bashes). Apply your first file change (\`edit\` or \`write\`) to the highest-priority target now. A partial patch always outscores an empty diff.`,
						"exploration-threshold-hit",
						12_000,
					);
					explorationCount = 0;
				}
				...
				if (
					...
					enqueueNudge(
						`Discovery stall: ${totalExplorationSteps} read/bash steps with no \`edit\`/\`write\` yet. The top-ranked target is \`${primary}\` — \`read\` it if needed, then change it immediately. Do not run more broad directory listing.`,
						"discovery-stall",
						12_000,
					);
