...
const MAX_DISCOVERY_KEYWORDS = 12;
const MAX_FILENAME_SCAN_KEYWORDS = 4;
const MAX_DISCOVERY_FILE_HITS = 18;
const FILENAME_SCAN_HIT_THRESHOLD = 8;
const MAX_DISCOVERY_RESULT_FILES = 12;
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		const filtered = [...keywords]
			...
			.slice(0, 30);
		const scoreKeyword = (kw: string): number => {
			let score = 0;
			if (paths.has(kw)) score += 4;
			if (kw.includes("/")) score += 2;
			if (/[._-]/.test(kw)) score += 1;
			if (/[A-Z]/.test(kw)) score += 1;
			return score + Math.min(kw.length, 40) / 100;
		};
		const prioritizedKeywords = [...filtered]
			.sort((a, b) => scoreKeyword(b) - scoreKeyword(a))
			.slice(0, MAX_DISCOVERY_KEYWORDS);
		if (prioritizedKeywords.length === 0 && paths.size === 0) return "";
		...
		for (const kw of prioritizedKeywords) {
			try {
				...
				const result = execSync(
					...
					{ cwd, timeout: 1200, encoding: "utf-8", maxBuffer: 2 * 1024 * 1024 },
				).trim();
				if (result) {
			...
			if (fileHits.size >= MAX_DISCOVERY_FILE_HITS) break;
		}

		const filenameHits = new Map<string, Set<string>>();
		if (fileHits.size < FILENAME_SCAN_HIT_THRESHOLD) {
			for (const kw of prioritizedKeywords.slice(0, MAX_FILENAME_SCAN_KEYWORDS)) {
				...
				try {
					const nameResult = execSync(
						...
						{ cwd, timeout: 900, encoding: "utf-8", maxBuffer: 1024 * 1024 },
					).trim();
					if (nameResult) {
				...
				if (filenameHits.size >= Math.ceil(MAX_DISCOVERY_FILE_HITS / 2)) break;
			}
		}
		...
		const sorted = [...fileHits.entries()].sort((a, b) => b[1].size - a[1].size).slice(0, MAX_DISCOVERY_RESULT_FILES);
